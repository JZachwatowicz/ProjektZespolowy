  ...
  return (
    <div className='flex gap-10 flex-wrap justify-center'>
      <div className="p-11 mb-20 flex-grow ">
        <h1 className="mb-8 text-center text-3xl text-stone-800 font-semibold">Lista Aktywności</h1>
          <div className=' grid grid-cols-3'>
            <div className="flex items-center input-group  d-flex justify-content-between">
              <input className="form-control p-3 rounded-l-lg w-4/5 border-1 w-max" 
                type="search" 
                placeholder="Search"
                value={search}
                onChange={onChangeSearch}
              />
              <button className="p-4 mr-2 border-1 rounded-r-lg  text-blue-400 hover:text-white border border-blue-200 hover:bg-blue-400 "  onClick={()=>{HandleSearch()}}> 
          ...
          <div className="pt-5 grid gap-x-2">
            {
              ...
              && (
                currentRecords.map(act => (
                  <>
                    <div className="grid grid-cols-6 border-2 rounded-lg p-3 pt-6 pb-6 shadow-md m-2 bg-white" key={act.id}>
                    ...
                    <div className="col-span-3 flex p-1 items-center break-words">{act.description}</div>
                      <div className=" flex justify-end items-center pr-2">
                        <div className={screenSize <= 1200 ? 'flex flex-col' : 'flex flex-row'}>
                          <button onClick={() => editActivityHandler(act.id)} className=" p-3 shadow-xl m-1 rounded-lg border-1 bg-gray-600 text-white hover:bg-gray-400 hover:text-black ">
                            Edytuj
                          </button>
                          <button onClick={() => deleteActivityHandler(act.id)} className="flex text-center items-center p-3 shadow-xl m-1 rounded-lg  text-red-700 hover:text-white border border-red-700 hover:bg-red-800 ">
