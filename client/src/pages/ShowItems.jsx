...
import AuthService from "../services/auth.service.js";
  ...
  function HandleSearch(){
    if(search === ""){
      ...
      switch(searchType){
        case "name":  setFiltered(items.filter(i => i.name.includes(search)));
        break;
        case "serial":setFiltered(items.filter(i => i.serial_number.includes(search)));
        break;
        case "date" : setFiltered(items.filter(i => i.possesion_date.includes(search)));
        break;
        default: setFiltered(items.filter(i => i.name.includes(search)));
      }
        ...
        <h1 className="mb-8 text-center text-3xl font-semibold">Lista Sprzętu</h1>
          <div className=' grid grid-cols-3 content-center'>
            <div className="flex items-center col-span-2">
              <input className="form-control  dark:text-black p-3 rounded-l-lg w-4/5  w-max" 
                type="search" 
                placeholder="Search"
                value={search}
              ...
              <button className="p-4 mr-2  rounded-r-lg text-white bg-blue-400  hover:bg-blue-600"  onClick={()=>{HandleSearch()}}> 
